...
bool s_touchDown[3] = { false, false, false };
bool s_mouseMoved = false;
sf::Vector2i s_touchPos;
sf::Texture *s_fontTexture =
...
unsigned int s_joystickId = NULL_JOYSTICK_ID;

const unsigned int NULL_JOYSTICK_BUTTON = sf::Joystick::ButtonCount;
unsigned int s_joystickMapping[ImGuiNavInput_COUNT];
...
void ProcessEvent(const sf::Event &event)
{
	if ( s_windowHasFocus )
	{
		...
		{
			s_mouseMoved = false;
			const int button = event.touch.finger;
			  ...
			  const sf::Color &color, float thickness)
{
	...
	draw_list->AddLine(ImVec2(a.x + pos.x, a.y + pos.y), ImVec2(b.x + pos.x, b.y + pos.y), toImColor(color), thickness);
}
...
{
	ImDrawList *draw_list = ImGui::GetWindowDrawList();
	draw_list->AddRect(getTopLeftAbsolute(rect), getDownRightAbsolute(rect), toImColor(color), rounding,
					...
					float rounding, int rounding_corners)
{
	...
	draw_list->AddRectFilled(
		getTopLeftAbsolute(rect), getDownRightAbsolute(rect), toImColor(color), rounding, rounding_corners);
}
...
void initDefaultJoystickMapping()
{
	GuiImpl::SFML::SetJoystickMapping(ImGuiNavInput_Activate, 0);
	GuiImpl::SFML::SetJoystickMapping(ImGuiNavInput_Cancel, 1);
	GuiImpl::SFML::SetJoystickMapping(ImGuiNavInput_Input, 3);
	GuiImpl::SFML::SetJoystickMapping(ImGuiNavInput_Menu, 2);
	GuiImpl::SFML::SetJoystickMapping(ImGuiNavInput_FocusPrev, 4);
	GuiImpl::SFML::SetJoystickMapping(ImGuiNavInput_FocusNext, 5);
	GuiImpl::SFML::SetJoystickMapping(ImGuiNavInput_TweakSlow, 4);
	GuiImpl::SFML::SetJoystickMapping(ImGuiNavInput_TweakFast, 5);

	GuiImpl::SFML::SetDPadXAxis(sf::Joystick::PovX);
	// D-pad Y axis is inverted on Windows
#ifdef _WIN32
	GuiImpl::SFML::SetDPadYAxis(sf::Joystick::PovY, true);
#else
	GuiImpl::SFML::SetDPadYAxis(sf::Joystick::PovY);
#endif

	GuiImpl::SFML::SetLStickXAxis(sf::Joystick::X);
	GuiImpl::SFML::SetLStickYAxis(sf::Joystick::Y);

	GuiImpl::SFML::SetJoytickDPadThreshold(5.f);
	GuiImpl::SFML::SetJoytickLStickThreshold(5.f);
}
