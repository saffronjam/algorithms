...
Algorithm::Algorithm(std::string name) :
	...
	_isActive(true),
	_nameTextFont(FontStore::Get("segoeui.ttf", false)),
	...
	_colorTransitionDuration(0.7f)
{
	...
	_palettes.push_back(ImageStore::Get("pals/rainbowLarge.png", false));
	_palettes.push_back(ImageStore::Get("pals/fieryLarge.png", false));
	_palettes.push_back(ImageStore::Get("pals/uvLarge.png", false));
...
void Algorithm::DrawName(Scene& scene, const sf::FloatRect& rect)
{
	...
	scene.Submit(_nameText, TextAlign::Middle, sf::RenderStates::Default);
}
...
void Algorithm::SoftResize(size_t size)
{
	...
	while (_elementsReset.size() != size)
	{
		...
		{
			_elementsReset.push_back(Element(generator(_elementsReset.size()), sf::Color::White));
		}
	}
	_elements = _elementsReset;
	_elementsRestart = _elementsReset;
...
{
	Reset();
	_image = ImageStore::Get(filepath, false);
...
auto Algorithm::Generator() -> std::function<long(size_t)>
{
	...
	Log::CoreError("Invalid NumberGeneratorType while creating element generator");
	return {};
}
...
auto Algorithm::HighestElementValue() -> long
{
	...
	Log::CoreError("Invalid NumberGeneratorType while calculating highest element value");
	return {};
}
...
void Algorithm::DrawBars(Scene& scene, const sf::FloatRect& rect)
{
	...
	scene.Submit(_barsVA, sf::RenderStates::Default);
}
...
void Algorithm::DrawNumberLine(Scene& scene, const sf::FloatRect& rect)
{
	...
	scene.Submit(_numberLineVA, sf::RenderStates::Default);
	for (size_t i = 0; i < _numberLineVA.getVertexCount() / 4; i++)
	{
		scene.Submit(_numberLineTextList[i], TextAlign::Middle, sf::RenderStates::Default);
	}
}
...
void Algorithm::DrawCircles(Scene& scene, const sf::FloatRect& rect)
{
	...
	scene.Submit(vertexArray, sf::RenderStates::Default);
}
...
void Algorithm::DrawHoops(Scene& scene, const sf::FloatRect& rect)
{
	...
	for (size_t i = 0; i < _elements.size(); i++)
	{
		...
		scene.Submit(_hoopsShapes[i], sf::RenderStates::Default);
	}
}
...
void Algorithm::DrawLine(Scene& scene, const sf::FloatRect& rect)
{
	...
	scene.Submit(line, sf::RenderStates::Default);
}
...
void Algorithm::DrawScatterPlot(Scene& scene, const sf::FloatRect& rect)
{
	...
	if (xMult < 1.0f)
	{
		...
		scene.Submit(scatterPlot, sf::RenderStates::Default);
	}
	else
	{
		...
		for (size_t i = 0; i < _elements.size(); i++)
		{
			...
			scene.Submit(circleShape, sf::RenderStates::Default);
		}
	}
}
...
void Algorithm::DrawImage(Scene& scene, const sf::FloatRect& rect)
{
	...
	scene.Submit(renderTextureSprite, sf::RenderStates::Default);
}
