...
namespace Se
{
RadixSort::RadixSort() : Algorithm("Radix Sort")
{
}
...
{
    long h = HighestValue();
    for (size_t exp = 1; (size_t) (h / exp) > 0 && _state != State::BeingCollected; exp *= 10)
        CountSort(exp);
}
...
void RadixSort::CountSort(size_t exponent)
{
    ...
    if (_state != State::BeingCollected)
        GetElements() = outBucket;
}

long RadixSort::HighestValue()
{
    return std::max_element(GetElements().begin(), GetElements().end(),
